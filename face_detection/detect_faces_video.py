...
# USAGE
# python detect_faces_video.py --prototxt model/deploy.prototxt.txt
...
def detect_faces_video(net):
	# initialize the video stream
	print("[INFO] starting video stream...")
	vs = VideoStream(src=0).start()

	# for Raspberry Pi and picamera :
	# vs = VideoStream(usePiCamera=True).start()

	# for video file (rather than a video stream) : swap out the VideoStream  class for FileVideoStream
	...
	while True:
		# grab frame from video stream and resize it to have a maximum width of 400 pixels
		frame = vs.read()
