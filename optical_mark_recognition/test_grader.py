...
def grade_test(image, key):
	# convert image to grayscale, blur it slightly to correct for noise, then find edges
	...
	# apply a four point perspective transform to both the original and grayscale to obtain a top-down view of the paper
	...
	for (q, i) in enumerate(np.arange(0, len(questionCnts), 5)):
		...
		for (j, c) in enumerate(cnts):
			...
			if bubbled is None or total > bubbled[0]:
				bubbled = (total, j)
			# !! if none is bubbled-in, then answer B is selected, because B is the "boldest" letter
...
if __name__ == '__main__':
	...
	grade_test(image, key=ANSWER_KEY)
